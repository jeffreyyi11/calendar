...
import getEvents from "./MinioCalls/getEvents";
...
const Calendar = () => {
  ...
  const eventsForDate = (date) => {
    return events.filter((event) => event["date"] === String(date));
  };

  //function to grab community events and set in state
  const getCommunityEvents = async(month) => {
    let communityEvents = await getEvents(month);
  ...
  return (
    <div id="calendar-container" className="ptb-100 calendarContainer">
      <CalendarHeader
      ...
      <div className="calendar">
        {days.map((d, index) => (
          <Days
            ...
            selectDate={() => {
              ///select a day to bring up modal to add event and view events for that day
              if (!d.padding) {
      ...
      <div id="events">
        <Events events={events} monthYear={monthYear} />
      </div>
      <div id="event-modal">
        {selectedDate && (
          <EventModal
